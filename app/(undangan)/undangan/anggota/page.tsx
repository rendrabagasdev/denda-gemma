...
export default function AnggotaPage() {
  ...
  const uniqueDivisis = useMemo(() => Array.from(new Set(members.map(m => m.divisions?.name || m.divisi || '-'))).sort(), [members])
  ...
  const filteredMembers = useMemo(() => {
    return members
      .filter(m => {
        ...
        const currentDivisi = m.divisions?.name || m.divisi || '-'
        const matchesDivisi = filterDivisi === 'all' || currentDivisi === filterDivisi
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Kelola data warga GEMMA</p>
            </div>
            <button 
              onClick={() => handleOpenModal()}
              className="cartoon-btn bg-primary px-8 h-14 flex items-center justify-center gap-3 font-black uppercase text-xs tracking-widest shadow-xl shadow-primary/20"
            >
                        ...
                        <tr key={member.id} className="group hover:bg-zinc-50/50 transition-colors">
                          ...
                          <td className="p-6">
                            <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-wider">
                              {member.divisions?.name || member.divisi || '-'}
                            </span>
                          </td>
                          <td className="p-6">
                    ...
                    <div key={member.id} className="p-4 flex items-center gap-4 active:bg-zinc-50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-2 mt-1">
                          ...
                          <span className="text-[9px] font-bold text-zinc-400 truncate max-w-[100px]">
                            {member.divisions?.name || member.divisi || '-'}
                          </span>
                        </div>
                      </div>
                ...
                <div className="pt-4 flex flex-col gap-3">
                  ...
                  <div className="flex items-center gap-2 justify-center text-zinc-400">
                    ...
                    <p className="text-[9px] font-bold uppercase tracking-widest">
                      Data akan otomatis disinkronkan ke modul Denda
                    </p>
                  </div>
                </div>
              </form>
